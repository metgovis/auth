...
exports.resetpassword = (args) => {
  ...
  try {
    ...
    transporter.sendMail(
      {
        context: {
          link: [
            ...
            args.result.userId,
            "&password=",
            args.result.password,
            "&appId=",
            __settings.client.appId,
            "&returl=",
            __settings.client.metgovisAppName,
            "/authenticate",
          ].join(""),
          name: [args.result.name.first, args.result.name.last].join(" "),
        },
        to: __settings.production
          ? args.result.email
          : __settings.smtp.auth.user,
        from: __settings.production
          ? "admin@metgovis.co.za"
      ...
      (error, info) => {
        if (error) {
          ...
          err.error.errors[0].reason =
            "Unable to send the password reset email. An unexpected error has occurred.";
    ...
    err.error.errors[0].code = 503;
    err.error.errors[0].reason =
      "Unable to send the password reset email. Please verify your SMTP details.";
    err.error.errors[0].message = error.message;
